...
lars_model1 = GridSearchCV(lars, lars_grid, cv =10, scoring = 'mean_absolute_error')
lars_model2 = GridSearchCV(lars, lars_grid, cv =10, scoring = 'mean_squared_error')
...
hreg_model = GridSearchCV(hreg, hreg_grid, cv=10, scoring = ['neg_mean_absolute_error', 'neg_mean_squared_error'])
hreg_model.fit(X_train, y_train)
filename5 = 'hreg.sav'
pickle.dump(hreg_model, open(filename5,'wb'))
...
sgd_load = pickle.load(open('sgd.sav', 'rb'))
print('SGD Regressor Mean Square:', sgd_load )


hreg_load = pickle.load(open('hreg.sav', 'rb'))
predict1 =hreg_load.predict(X_test)
print('Huber Regression:', r2_score(y_test, predict1))


kridge_load = pickle.load(open('kridge.sav', 'rb'))
predict2 =kridge_load.predict(X_test)
print('Kernel Ridge Regression:', r2_score(y_test, predict2))


gbr_load = pickle.load(open('gbr.sav', 'rb'))
predict3 =gbr_load.predict(X_test)
print('Gradient Boosting Regressor:', r2_score(y_test, predict3))


knn_load = pickle.load(open('knn.sav', 'rb'))
predict4 =knn_load.predict(X_test)
print('KNN:', r2_score(y_test, predict4))

lars_load = pickle.load(open('lars_model.sav', 'rb'))
predict5 =lars_load.predict(X_test)
print('Least Angle Regressor:', r2_score(y_test, predict5))


dtree_load = pickle.load(open('dtree.sav', 'rb'))
predict6 =dtree_load.predict(X_test)
print('Decision Tree Regressor:', r2_score(y_test, predict6))

lass_load = pickle.load(open('lass_grid.sav','rb'))
predict7 = lass_load.predict(X_test)
print('Lasso Regressor:', r2_score(y_test, predict7))

lr_load = pickle.load(open('lr_grid.sav','rb'))
predict8 = lr_load.predict(X_test)
print('Linear Regression:', r2_score(y_test, predict8))


ridge_load = pickle.load(open('ridge_grid.sav','rb'))
predict9 = ridge_load.predict(X_test)
print('Ridge Regression:', r2_score(y_test, predict9))

ba_load = pickle.load(open('ba_reg.sav','rb'))
predict11 = ba_load.predict(X_test)
print('Bayessian:', r2_score(y_test, predict11))

        
svm_load = pickle.load(open('svr.sav','rb'))
predict11 = svm_load.predict(X_test)
print('Bayessian:', r2_score(y_test, predict11))



coefficients = pd.DataFrame({"AFeature":X.columns,"Coefficients_Bayesian":np.transpose(ba_load.coef_),
                                "Coefficients_Lasso":np.transpose(lass_load.best_estimator_.coef_),
                                "Coefficients_Ridge":np.transpose(ridge_load.best_estimator_.coef_),
                                "Coefficients_LinerReg":np.transpose(lr_load.best_estimator_.coef_),
                                "Coefficients_LeastAngle":np.transpose(lars_load.best_estimator_.coef_),
                                "Coefficients_HuberReg":np.transpose(hreg_load.best_estimator_.coef_)
                                #"Coefficients_DecisionTree":np.transpose(dtree_load.best_estimator_.coef_),
                                #"Coefficients_KNN":np.transpose(knn_load.best_estimator_.coef_)
                                #"Coefficients_SVM":np.transpose(svm_load.coef_)
                                })
